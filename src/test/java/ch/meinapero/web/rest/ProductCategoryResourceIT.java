...
class ProductCategoryResourceIT {
    ...
    void getAllProductCategoriesWithEagerRelationshipsIsEnabled() {
        when(productCategoryServiceMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());
    ...
    @SuppressWarnings({ "unchecked" })
    void getAllProductCategoriesWithEagerRelationshipsIsNotEnabled() {
        when(productCategoryServiceMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());

        webTestClient.get().uri(ENTITY_API_URL + "?eagerload=false").exchange().expectStatus().isOk();
        verify(productCategoryRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
